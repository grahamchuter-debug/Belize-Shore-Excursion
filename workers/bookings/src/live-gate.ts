...
export function liveReadinessGaps(
  ...
  if (!(env.SITE_BASE_URL || "").includes("belizeshoreexcursion.com")) gaps.push("site_base_url");
...
export function liveCheckoutBlock(
  ...
  if (!LIVE_PAYMENTS_CODE_ENABLED) {
    return {
      ...
      message: "Live Stripe is not enabled for Belize. The live code flag is off. No payment was taken.",
    };
  }
  ...
  if (!site.includes("belizeshoreexcursion.com")) {
    return {
      code: "SITE_BASE_URL",
      message: "Live Checkout requires SITE_BASE_URL on belizeshoreexcursion.com. No payment was taken.",
    };
  }
  if (!product) {
    return {
