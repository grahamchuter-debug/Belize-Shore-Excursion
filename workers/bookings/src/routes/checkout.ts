...
export async function handleCreateCheckout(request: Request, env: Env): Promise<Response> {
    ...
    return jsonResponse(
      {
        ...
        message: "Belize online booking is switched off. No payment was taken.",
      },
      503,
    );
  }
  ...
  if (product.id === "belize-cave-tubing" && !body.eligibilityAcknowledged) {
    return jsonResponse(
      {
        ok: false,
        code: "ELIGIBILITY",
        message: "Please confirm all participants are at least 8 years old and at least 48 inches tall.",
      },
      400,
    );
  }
    ...
    const session = await stripe.checkout.sessions.create(
      {
        ...
        // Belize: hide Link/OneLink wallet so card entry is the primary path.
