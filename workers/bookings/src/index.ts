/**
 * Belize request-to-book Worker (shared for three Belize RTB products).
 *
 ...
 */
import { belizeBookingCore } from "../../../shared/destinations/belize";
...
async function handlePreviewRequest(request: Request, env: Env): Promise<Response> {
    ...
    return jsonResponse(
      {
        ...
        message:
          "Live Stripe is not enabled for this Belize Worker. Charge-then-refund still requires an explicit production configuration. No payment was taken.",
      },
      503,
    );
  }
  ...
  const product = findBelizeBookingProduct(body.productId);
  ...
  const reference = createBookingReference(belizeBookingCore);
  ...
  const destinationBrand = destinationBrandFromCore(belizeBookingCore);
